...
plot3 <- function (rds_path = "") {
  ...
  with(comdata1, plot(Time,Sub_metering_1,type ="l",ylim = c(0, 40),yaxp = c(0, 30, 3), ylab = "Energy sub metering", xlab ="",cex.axis=0.8, cex.lab=0.8))
  with(comdata1, lines(Time,Sub_metering_2,col ="RED"))
  with(comdata1, lines(Time,Sub_metering_3,col ="BLUE"))
  legend("topright",col = c("black","red","blue"), legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lty=c(1,1,1),cex=0.8)
  dev.off()
  message("Graphic saved as plot3.png in the current working directory")
}
